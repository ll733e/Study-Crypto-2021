...
inline void u4byte_out(u8 *x, u32 y)
{
    x[0] = (y >> 24) & 0xFF;
    x[1] = (y >> 16) & 0xFF;
    x[2] = (y >> 8) & 0xFF;
...
inline void AES_KeyWordToByte(u32 W[], u8 RK[])
{
    int i;
    for(i = 0 ; i < 44 ; i++)
...
inline void RoundkeyGeneration128(u8 MK[], u8 RK[])
{
    u32 W[44];
    int i;
    u32 T;
...
inline void AES_KeySchedule(u8 MK[], u8 RK[], int keysize)
{
...
inline void AddRoundKey(u8 S[16], u8 RK[16])
{
...
inline void Mixcolumns(u8 S[16])
{
    u8 temp[16];
...
inline void AES_ENC(u8 PT[16], u8 RK[16],u8 CT[16], int keysize)
{
    int Nr = keysize / 32 + 6;
    int i;
